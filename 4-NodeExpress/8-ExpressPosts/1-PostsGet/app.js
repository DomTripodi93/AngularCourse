...
app
    ...
    .put("/post/postEdit", (req, res) => {
        editPost(req, res);
    })
    .post("/post/postAdd", (req, res) => {
        addNewPost(req, res);
    })
    .delete("/post/postDelete/:postId", (req, res) => {
        deletePost(req, res);
    })
...
function addNewPost(req, res) {
    fs.readFile("posts.json", { encoding: 'utf-8' }, (err, fileResult) => {
        ...
        newPost.postId = postList[postList.length - 1].postId + 1;
        postList.push(req.body);

        writeFile(postList, "posts").then(() => {
            res.send(newPost);
        })
    });
}
...
function editPost(req, res) {
    fs.readFile("posts.json", { encoding: 'utf-8' }, (err, fileResult) => {
        ...
        postFromBody.updateDate = new Date();

        const postFromFile = getPostById(postList, +req.body.postId)
        let postIndex = postList.indexOf(postFromFile);

        postList.splice(postIndex, 1, postFromBody)

        writeFile(postList, "posts").then(() => {
            res.send(postFromBody);
        })
    });
}
...
function deletePost(req, res) {
    fs.readFile("posts.json", { encoding: 'utf-8' }, (err, fileResult) => {
        ...
        const postFromFile = getPostById(postList, +req.params.postId)
        let postIndex = postList.indexOf(postFromFile);
        postList.splice(postIndex, 1)

        writeFile(postList, "posts").then(() => {
            res.send({ deleted: req.params.postId });
        })
    });
}


function getPostById(postList, postId) {
    return postList.filter(record => {
